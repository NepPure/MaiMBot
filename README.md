...
> ⚠️ **警告**：请自行了解qqbot的风险，爱酱有时候一天被腾讯肘七八次
> ⚠️ **警告**：由于爱酱一直在迭代，所以可能存在一些bug，请自行测试，包括胡言乱语（

关于爱酱的开发和部署相关的讨论群（不建议发布无关消息）
...
2. **MongoDB设置**
   - 安装并运行mongodb
   - 爱酱bot会自动连接默认的mongodb，端口和数据库名可配置
...
### 💬 聊天功能
- 支持关键词检索主动发言：对消息的话题topic进行识别，如果检测到爱酱存储过的话题就会主动进行发言，目前有bug,所以现在只会检测主题，不会进行存储
- 支持bot名字呼唤发言：检测到"爱酱"会主动发言，可配置
- 使用硅基流动的api进行回复生成，可随机使用R1，V3，R1-distill等模型，未来将加入官网api支持
- 动态的prompt构建器，更拟人
- 支持图片，转发消息，回复消息的识别
- 错别字和多条回复功能：爱酱可以随机生成错别字，会多条发送回复以及对消息进行reply
